...
- **The emotional journal (`/journal`)** records check-ins — one to five feelings from a fixed vocabulary, each with a strength and what it was about: a person, a **trigger** (two halves: *who or what* and *what happened*, so *Lucie · meeting* and *meetings with anyone* are both readable later), or a context tag. A voice check-in is transcribed and labelled **on the device**; the model's proposal is shown dashed with the **quote** from the note that supports each feeling, and nothing is saved until the user keeps it. **Insights (`/journal/insights`)** reads the journal as drift: where each person, trigger or pair sits on three axes, how far each has moved, which feelings it brings, and the weeks — computed on read, drawn as SVG, stored nowhere. See [docs/01-concepts.md](docs/01-concepts.md) and [docs/06-frontend.md](docs/06-frontend.md).
