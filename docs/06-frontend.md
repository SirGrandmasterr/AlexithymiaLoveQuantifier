...
| [`journal/embeddings/embed.js`](../src/journal/embeddings/embed.js) | 279 | **The embedding boundary** (§5.8, G1): `embedTexts(texts, { kind, runtime })` with the runtime injected, `prefixed()` — the **only** way a string reaches the model, because the two prompt prefixes are mandatory and a wrong one has no symptom — and `toIndexVector`, Matryoshka truncation to 256 that **re-normalises**. `createWebEmbedder` is EmbeddingGemma through transformers.js behind a dynamic import. |
| [`journal/embeddings/store.js`](../src/journal/embeddings/store.js) | 241 | **The device-local index** (§5.8 rule 1): `{ entry_client_id, model, dims, vector }` in IndexedDB behind an injectable backend, `staleIds` for a model change, `clearVectorIndex` for a logout. No server endpoint, no export path, no HNSW. |
| [`journal/embeddings/similar.js`](../src/journal/embeddings/similar.js) | 293 | **The scan and the three rules**, pure: `cosine`, `scan`, `buildWitnesses` and `witnessAgrees` — rule 3 as a **hard gate**, not a weight — and the two things the card and the Triggers view may offer, each returning labels and ids with the similarity thrown away (rule 2). |
| [`journal/embeddings/recall.js`](../src/journal/embeddings/recall.js) | 615 | **§5.8's remaining four uses**, pure (G2). `buildDocuments` over entries *and* snapshot notes; `lexicalRank`, weighted by inverse document frequency over the user's own corpus rather than by a stopword list, so no language is named anywhere in the file; `recall`, which keeps what was **found** apart from what merely **looks alike**; `pastEntryOffers`, `orderNamesakes`, `alreadyKnown`, and `retrievalVocabulary` — which reads triggers and people and **never feelings**, because that is the echo channel. |
| [`journal/embeddings/retrievalGolden.js`](../src/journal/embeddings/retrievalGolden.js) | 256 | The retrieval golden set and its scorer, shared by `npm test` and `make journal-eval`. Lexical cases run with no model; semantic ones are reported **skipped, by name**, never graded against a stand-in. |
| [`journal/embeddings/golden/`](../src/journal/embeddings/golden/) | — | `retrieval.json` — a fixture journal in two languages, two snapshot notes and 26 cases. See its [README](../src/journal/embeddings/golden/README.md). |
| [`journal/embeddings/EmbeddingContext.jsx`](../src/journal/embeddings/EmbeddingContext.jsx) | 358 | The index as one object the screens ask questions of, mounted **inside** `JournalProvider`. Keeps the vocabulary and the entries embedded lazily; answers `offersFor`, `search`, `pastFor`, `orderCandidates` and `vocabularyFor` — all with labels, ids and entries, never a number. **A component with no provider gets the feature off**, not an error. |
| [`journal/embeddings/availability.js`](../src/journal/embeddings/availability.js) | 31 | Whether this device can keep an index at all: IndexedDB, and not the Android shell — written as a **missing runtime** rather than a platform rule, so the session that adds one deletes a condition. |
| [`journal/embeddings/embed.fake.js`](../src/journal/embeddings/embed.fake.js) | 126 | A fake embedder that records every string it was handed, prefix and all, and lets a test place two texts a stated cosine apart. **Tests only.** |
...
| [`JournalSearch.jsx`](../src/components/JournalSearch.jsx) | 236 | `/journal/search` — §5.8's recall, entirely on this device and behind the index toggle. **Two lists, not one**: *entries with these words* is a fact the reader can check; *entries with similar words* is this device's guess, under its own heading. Results are entries — a day, a time, the user's own words, the feelings they were filed under — and the app never summarises them. |
...
> **Seven claims is what §10.2 legislates; invariant 2e is about every sentence on the page.**
> The Phase 6 closeout walked all twenty of them against the code, in both opt-in states and
> on all three tiers, and found every one true — and seven of them, all predating Phase 6,
> asserted by nothing. *"Who can see this?"*, the export and import paragraphs, the CSV
> footnote, the lock's scope and its idle limit, *"Unavailable here"*, and the footer's
> session count are now held verbatim too, so `Vault.test.jsx` is **37 tests** rather than 30.
> The rows below are still the seven that carry a model or a network claim; they are the ones
> most likely to become false, not the only ones that must stay true.
...
| "Nothing is merged or renamed unless you tap it" — *index on* | Similarity **proposes and never writes** (§5.8 rule 2). Accepting *"you've called this 'work' before"* makes the check-in reference a trigger the user already has instead of minting a new one — which is what §4.5b step 1 would have done had the label matched exactly — and writes no correction row: the test asserts no `merged_into` and no `corrects` in any body. The Triggers view's *looks similar to…* opens the same `MergeTriggerDialog` the row's own action opens, with the same radio and the same one-way sentence, so the suggestion changes which pair is easy to find and not what a tap does. **G2 added three more things similarity may propose and still not write**: the past-entry chips are dashed and carry `from: "retrieval"` with the ids they were read from; the namesake ordering returns the *same array* §4.5 produced, reordered, and `orderNamesakes` cannot add, remove or select a candidate; and the proposal prompt's retrieval context can only move the user's own confirmed words to the front of two lists. Rule 2's other half — **never a number** — is the return type: `similarTriggerOffers` and `pastEntryOffers` both throw the similarity away, and `journal.test.js` walks `JOURNAL_COPY.similar` — search, past entries, namesakes and *already known* included — for digits |
| "Search happens here and asks the server nothing" — *index on* | There is no search endpoint and G2 added none: `recall` runs over the entries the provider already holds and the vectors this device already built. `retrieval.test.jsx` types a query and asserts `axios.post` was never called and that no `GET` names a search, a vector or an embedding. The companion clause — *"search finds nothing until this device has read your entries back"* — is the same logout branch: the index is emptied with the outbox, and it rebuilds lazily from the entries the next session loads |
| "The journal cannot be searched" — *index off, the default* | `/journal/search` is behind the `alq:journal-embeddings` switch and renders a sentence naming that switch instead of a search box. Half of `recall` — the lexical half — would work without any model, and it is deliberately **not** offered separately: §9.7 gives this one control, and a search that quietly worked while the toggle said off would make both this page and the settings row untrue |
...
**Every §9.7 setting now has a control.** The last of the nine was `embeddings`, which arrived
with G1 — off by default, refused where no index could exist, with the download, its size and
its **licence** line beside it, because EmbeddingGemma is not Apache. G1 shipped its label
narrowed to *"Similar-entry suggestions"* rather than §9.7's *"Similar-entry suggestions and
search"*, since a toggle may not promise a screen the build does not have (invariant 2e); **G2
built that screen — `/journal/search`, behind this same switch — and the row is whole again**.
`Profile.test.jsx` asserts the full string, so a later session removing search would be told to
shrink the label with it. (Until C3 the older version of this paragraph covered voice,
