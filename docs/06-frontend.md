...
| "One model, and it does not run here" — *the Gemini option on* | The fourth variant of the *AI features* answer, and the only state in which this page cannot say *on this device* about anything. It is a separate paragraph rather than an edit to the other three because every promise they make is a promise about locality, and each is false here differently: the model is not open weights, it was not downloaded, it does not run here, and **the audio is sent**. `aiClaimFor(tier, cloud)` picks it and `Vault.test.jsx` asserts it verbatim. What survives unchanged is what the code still makes true — the same prompt (rule 3: only what was said), `buildProvenance` reading the card's **confirmed** state, and the switch in the profile |
| "One thing, because you turned it on" — *the Gemini option on* | The *send* answer's second variant. The first sentence of the default one is *"Nothing."*, and with a recording in flight that is false in the way this page is least allowed to be wrong. What stays exactly true is the half a user can check: **the browser** still talks only to this origin — `connect-src 'self'` is unchanged and the hop is the server's, from one endpoint, carrying the recording and nothing else. `SEND_CLAIM.cloud` and `SEND_CLAIM.off`, both asserted |
| Both Gemini variants are conditional on **the server as well as the switch** | `cloudIsOn()` reads three things: `alq:journal-cloud`, the answer to `GET /api/journal/propose/status`, and whether this device can hold a microphone open. A `true` left behind by a browser talking to a server that has since lost its key cannot make this page describe a hop that is not happening — the same rule `voiceIsOn()` follows for the tier, one layer out. The settings screen goes further and **rewrites** the refused `true`, so the switch on screen and the claim on this page cannot drift |
